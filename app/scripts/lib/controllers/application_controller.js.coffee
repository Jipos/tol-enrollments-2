define [
  ...
  class ApplicationController extends Marionette.Controller
    ...
    constructor: (options = {}) ->
      @region = options.region or msgBus.request 'default:region'
      super options
      @_instance_id = _.uniqueId('controller')
      msgBus.execute 'register:instance', @, @_instance_id

    close: ->
      console.log 'closing', @
      msgBus.execute 'unregister:instance', @, @_instance_id
      super
    ...
    _manageView: (view, options) ->
      if options.loading
        ...
        msgBus.execute 'show:loading', view, options
      else
